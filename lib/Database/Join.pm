...
=item SQLite backend writes a temporary file per query call

When the SQLite path is active, a new C<.db> file with a randomly generated
name (chosen by C<File::Temp> to avoid collisions) is created in C<tmpdir> at
the I<start> of each call to C<selectall_arrayref()>, C<selectall_array()>,
C<fetchrow_hashref()>, or C<count()>.  The file is deleted at the I<end> of
that same call, before the method returns -- even if an exception is raised.
The file never persists between calls or after the method returns.

If you make ten query calls on the same object, ten temporary files are created
and deleted one at a time, each lasting only for the duration of its own call.
At any given moment no more than one such file exists per C<Database::Join>
object.  The directory must be writable and have enough free space for one such
file.
...
Each query call that uses the SQLite path creates one temporary C<.db> file in
C<tmpdir> (default: C<File::Spec-E<gt>tmpdir()>, usually C</tmp> on Unix).
The filename is randomly generated by C<File::Temp> -- you cannot predict it,
only the directory is under your control.  If the directory is not writable,
or the filesystem is full, the call will C<croak> with C<error_sqlite_connect>.
Check permissions and free space if you see that error.
...
A new file is created at the start of each query call
(C<selectall_arrayref>, C<fetchrow_hashref>, etc.) and deleted at the end of
that same call, before the method returns -- even if an error occurs.  The
file has a randomly generated name in C<tmpdir> and never persists beyond
the call that created it.  See I<Temporary file: name, location, and lifetime>
below for details.
...
When the SQLite path is active, a temporary SQLite database file is created at
the I<start> of each query call (C<selectall_arrayref>, C<selectall_array>,
C<fetchrow_hashref>, C<count>).  The file is deleted at the I<end> of that
same call, before the method returns -- even if an exception occurs.
...
B<Lifetime: one file per query call, deleted before the call returns>: a new
file is created at the start of every query call that uses the SQLite path and
deleted before that call returns.  Calling C<selectall_arrayref()> ten times
creates and deletes ten files, one at a time.  At most one such file exists at
any moment per C<Database::Join> object.  The file never persists between calls
or after the method returns.
