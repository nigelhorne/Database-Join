...
- SQLite backend writes a temporary file per query call

    When the SQLite path is active, a new `.db` file with a randomly generated
    name (chosen by `File::Temp` to avoid collisions) is created in `tmpdir` at
    the _start_ of each call to `selectall_arrayref()`, `selectall_array()`,
    `fetchrow_hashref()`, or `count()`.  The file is deleted at the _end_ of
    ...
    At any given moment no more than one such file exists per `Database::Join`
...
- Temp file directory must be writable and have free space
    ...
    Each query call that uses the SQLite path creates one temporary `.db` file in
    `tmpdir` (default: `File::Spec->tmpdir()`, usually `/tmp` on Unix).
    ...
    or the filesystem is full, the call will `croak` with `error_sqlite_connect`.
...
- `backend => 'sqlite'` -- SQL JOIN via a temporary file
    ...
    (`selectall_arrayref`, `fetchrow_hashref`, etc.) and deleted at the end of
    that same call, before the method returns -- even if an error occurs.  The
    file has a randomly generated name in `tmpdir` and never persists beyond
    the call that created it.  See _Temporary file: name, location, and lifetime_
    below for details.
...
the _start_ of each query call (`selectall_arrayref`, `selectall_array`,
`fetchrow_hashref`, `count`).  The file is deleted at the _end_ of that
...
**Lifetime: one file per query call, deleted before the call returns**: a new
file is created at the start of every query call that uses the SQLite path and
deleted before that call returns.  Calling `selectall_arrayref()` ten times
creates and deletes ten files, one at a time.  At most one such file exists at
any moment per `Database::Join` object.  The file never persists between calls
